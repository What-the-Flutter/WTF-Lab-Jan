...
import '../entity/list_messages.dart';
...
class _ChatPage extends State<ChatPage> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: eventSelected
          ? _appBarFromChatPage()
          : _appBarWhenSelectedFromChatPage(indexOfSelectedElement),
      body: Column(
        children: [
  ...
  AppBar _appBarFromChatPage() => AppBar(
        backgroundColor: Colors.black,
        leading: const BackButton(),
        title: Center(child: Text(widget.category.title)),
        actions: [
          IconButton(
            onPressed: () {},
  ...
  AppBar _appBarWhenSelectedFromChatPage(int index, {int? count}) {
    return AppBar(
      ...
      leading: IconButton(icon: const Icon(Icons.clear), onPressed: _swapAppBar),
      actions: <Widget>[
        IconButton(
        ...
        IconButton(icon: const Icon(Icons.bookmark_border), onPressed: _swapAppBar),
        IconButton(
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              Text(
                '\nAdd your first event to ${widget.category.title} page by entering some text in the box bellow and hitting the send button. Long tap the send button to align the event in the opposite direction. Tap on bookmark icon on the top right corner to show the bookmarked events only.',
          ...
          children: [
            ...
            IconButton(
              ...
              onPressed: () {
                if (isEditing) {
                  setState(() {
                    _editText(indexOfSelectedElement);
                  });
                } else {
  ...
  void _sendMessage() {
    category.listMessages.insert(
      ...
      Message(1, DateTime.now(), _textEditingController.text),
    );
  ...
  void _copyMessage(int index) {
    Clipboard.setData(ClipboardData(text: category.listMessages[index].text));
  }
