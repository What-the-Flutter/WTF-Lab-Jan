...
class _MessageListPageState extends State<MessageListPage> {
  ...
  final Map<String, IconData> _eventsMenuItems = {
    'Sport': Icons.directions_run,
    'Movie': Icons.local_movies,
    'Music': Icons.music_note,
    'Food': Icons.local_dining,
  };
  final Map<String, IconData> _photoMenuItems = {
  ...
  void _onNewMessageButtonPress() {
    ...
    var currentTime =
        _isMessageEdited ? _selectedMessages[0].date : DateTime.now();
    setState(
      () => _allMessages.add(
        Message(
  ...
  void _onEditButtonPress() {
    setState(() {
      _isMessageEdited = !_isMessageEdited;
    ...
    return Visibility(
      child: SizedBox(
        ...
        child: ListView.separated(
          padding: const EdgeInsets.all(8),
          scrollDirection: Axis.horizontal,
          itemCount: _eventsMenuItems.length,
          itemBuilder: (_, index) {
            final item = _eventsMenuItems.entries.elementAt(index);
            return GestureDetector(
    ...
    return Container(
      ...
      child: Row(
        children: [
          IconButton(
            icon: Icon(
              _currentEvent == null ? Icons.bubble_chart : _currentEvent!.value,
            ),
