...
class _StatisticsPageState extends State<StatisticsPage> {
  ...
  void didChangeDependencies() {
    _halfScreenSize = MediaQuery.of(context).size.width / 2;
    super.didChangeDependencies();
  }

  @override
  Widget build(BuildContext context) {
    return BlocBuilder<StatisticsPageCubit, StatisticsPageStates>(
  ...
  List<charts.Series<Statistics, String>> createTodayDate() {
  ...
  List<Statistics> _generate(Function(int) func, int difference,
    ...
    while (difference != limit) {
      return _generate(func, difference - 1, statisticsList, limit, period);
    }
  ...
  List<charts.Series<Statistics, String>> _chartsSeries(
    ...
    return [
      charts.Series<Statistics, String>(
        ...
        data: _generate(countNotes, value, <Statistics>[], limit, period),
      ...
      charts.Series<Statistics, String>(
        id: 'events',
        measureFn: (sales, _) => sales.value,
        data: _generate(countEvents, value, <Statistics>[], limit, period),
      ...
      charts.Series<Statistics, String>(
        id: 'bookmarks',
        measureFn: (sales, _) => sales.value,
        data: _generate(countBookmarks, value, <Statistics>[], limit, period),
  ...
  Widget _eventsTileForCurrentPeriod() {
    ...
    switch (BlocProvider.of<StatisticsPageCubit>(context).state.period) {
      case 'Today':
        widget = _infoTile(_halfScreenSize - 15, _countEvents(1, 0), 'Events');
        break;
      case 'Past 7 days':
        widget = _infoTile(_halfScreenSize - 15, _countEvents(8, 0), 'Events');
        break;
      case 'Past 30 days':
        widget = _infoTile(_halfScreenSize - 15, _countEvents(31, 0), 'Events');
        break;
      case 'Past year':
        widget = _infoTile(_halfScreenSize - 15, _countEvents(0, 1), 'Events');
        break;
    }
    return widget;
  }
    ...
    switch (BlocProvider.of<StatisticsPageCubit>(context).state.period) {
      case 'Today':
        listOfWidgets = <Widget>[
          _infoTile(_halfScreenSize - 15, _countNotes(1, 0), 'Notes'),
          _infoTile(_halfScreenSize - 15, _countBookmarks(1, 0), 'Bookmarks'),
        ];
        break;
      case 'Past 7 days':
        listOfWidgets = <Widget>[
          _infoTile(_halfScreenSize - 15, _countNotes(8, 0), 'Notes'),
          _infoTile(_halfScreenSize - 15, _countBookmarks(8, 0), 'Bookmarks'),
        ];
        break;
      case 'Past 30 days':
        listOfWidgets = <Widget>[
          _infoTile(_halfScreenSize - 15, _countNotes(31, 0), 'Notes'),
          _infoTile(_halfScreenSize - 15, _countBookmarks(31, 0), 'Bookmarks'),
        ];
        break;
      case 'Past year':
        listOfWidgets = <Widget>[
          _infoTile(_halfScreenSize - 15, _countNotes(0, 1), 'Notes'),
          _infoTile(_halfScreenSize - 15, _countBookmarks(0, 1), 'Bookmarks'),
        ];
    }
  ...
  TextStyle _textStyle(String period) {
    return BlocProvider.of<StatisticsPageCubit>(context).state.period == period
        ? BlocProvider.of<ThemeCubit>(context).state.isLightTheme
  ...
  Widget _infoTile(double width, int count, String element) {
    return Container(
